    ...
    def __init__(self,
        ...
        while expand:
            ...
            for local_patch in local_patches:
                if any(
                        p.state is SCPatch.ROUTE for p in local_patch.adjacent(
                                None,
                                bound=False,
                                probe=False
                            )
        ...
        if vertical:
            if i + 1 < self.shape[0]:
                if ((self.graph[i, j].state is SCPatch.ROUTE)
                    or (self.graph[i + 1, j].state is SCPatch.ROUTE)
                    ):
                    opt.append([i + 1, j])

            if i - 1 >= 0:
                if (
                    (self.graph[i, j].state is SCPatch.ROUTE)
                    or (self.graph[i - 1, j].state is SCPatch.ROUTE)
                    ):
                    opt.append([i - 1, j])
        for idx in opt:
